...
\end{proposition}
\begin{proof}
First of all we have to define $a-norm$ of a vector, such that:
\begin{equation*}
\begin{aligned}
 ||x||_a:= |\{x_i| x_i >a_i\}| \\
 ||x||_b:= |\{x_i| x_i <b_i\}| 
...
\begin{equation}
\Delta_k= \min \{ x^k_{j(k)}-a_{j(k)}, b_{i(k)}-x^k_{i(k)}\}
\end{equation}

Then, once renaming $i(k)=i,j(k)=j$, at iteration $k+1$ we have:
\begin{equation*}
x^{k+1}_{j}=\begin{cases}
 a_j \ &x^k_{i}\le b_i+a_j-x^k_{j}\\
 x^k_{j}-b_i+x^k_{i} \ &x^k_{i}> b_i+a_j-x^k_{j}
 \end{cases}
\end{equation*}
and:
\begin{equation*}
x^{k+1}_{i}=\begin{cases}
 x^k_{j}-a_j+x^k_{i} \ &x^k_{i}\le b_i+a_j-x^k_{j}\\
 b_i \ &x^k_{i}> b_i+a_j-x^k_{j}
 \end{cases}
\end{equation*}

Hence we have the follwing cases:
\begin{equation}
 ||x^{k+1}||_a=\begin{cases} ||x_{k}||_a\hspace{2cm} x^k_{i}> b_i+a_j-x^k_{j} \\
 ||x_k||_a-1\hspace{2cm} else               
              \end{cases}
\end{equation}
and:
\begin{equation}
 ||x^{k+1}||_b=\begin{cases} ||x_{k}||_b\hspace{2cm} x^k_{i}\le b_i+a_j-x^k_{j} \\
 ||x_k||_b-1\hspace{2cm} else               
              \end{cases}
\end{equation}

Because of the sequences of $||\cdot||_a, ||\cdot||_b$ are not increasing.
