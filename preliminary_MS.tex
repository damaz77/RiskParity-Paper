...
\nabla_xf(\bar x,\bar y)^Td_x\ge 0\quad\quad \forall d_x\in R^n \ s.t. \mathds{1}^Td_x=0, d_i\ge 0 \ \forall i \in L(\bar x), d_i\le 0 \ \forall i \in U(\bar x)
\end{equation}
...
\begin{equation}\label{mvp}
 \nabla f(\bar x,\bar y)^Td^{i^\star,j^\star}\le \nabla f(\bar x,\bar y)^Td^{i,j} \forall i\in R(\bar x), \ j\in S(\bar x).
\end{equation}
